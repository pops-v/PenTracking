...
def calculate_angle(center, point):
    ...
    if angle_deg < 0:
        angle_deg += 360  # Ensure angle is in 0-360 range
    return angle_deg
        ...
        if contours:
            ...
            if cv2.contourArea(largest_contour) > 500:  # Filter small objects
                ...
                if M["m00"] > 0:
                    ...
                    # Calculate the angle of the pen relative to the center of the frame
                    frame_center = (frame.shape[1] // 2, frame.shape[0] // 2)
                    angle = calculate_angle(frame_center, (center_x, center_y))
                    cv2.putText(frame, f"Angle: {angle:.02f}°", (center_x, center_y + 20), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

                    # Draw on the canvas only if the angle is between 135° and 180°
                    if prev_x is not None and prev_y is not None and 10 < depth < 100:  # Adjust depth range as needed
                        if 135 <= angle <= 180:  # Check if the angle is within the desired range
                            cv2.line(canvas, (prev_x, prev_y), (center_x, center_y), (0, 255, 0), 5)
                    
                    # Print previous x and y coordinates
                    print(f"Previous Position: (x={prev_x}, y={prev_y}, z={depth}), Angle: {angle:.02f}°")
